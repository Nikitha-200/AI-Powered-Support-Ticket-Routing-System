...
const TicketForm = () => {
  ...
  const onFinish = async (values) => {
    setLoading(true);
    try {
      const response = await axios.post('http://localhost:5000/api/tickets', values);
  ...
  return (
    <div>
      ...
      <Card style={{ boxShadow: '0 6px 20px rgba(0,0,0,0.08)', borderRadius: 12 }}>
        <Title level={2} style={{ textAlign: 'center' }}>Submit Support Ticket</Title>
      ...
      {submittedTicket && (
        <Card style={{ marginTop: 20, borderColor: '#52c41a', boxShadow: '0 6px 20px rgba(0,0,0,0.08)', borderRadius: 12 }} title="Ticket Created">
            <Paragraph>
            ...
            <Button onClick={() => setSubmittedTicket(null)}>Submit Another</Button>
        </Card>
      )}
        </Col>
      </Row>
